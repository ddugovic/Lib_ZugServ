...
public abstract class SimpleRoomManager implements SimpleRoomListener {
	...
	public JsonNode serializeAllRooms(boolean deep) {
		ObjectNode obj = mapper.createObjectNode();
		ArrayNode roomlist = mapper.createArrayNode();
		for (SimpleRoom room: rooms) {
			if (deep) roomlist.add(room.serialize());
			else {
				ObjectNode roomObj = mapper.createObjectNode();
				roomObj.put("id", room.getID());
				roomObj.put("title", room.getTitle());
				roomObj.put("color", SimpleRoom.colorString(room.getColor()));
			}
		}
		if (deep) obj.set("roomlist_deep", roomlist); else obj.set("roomlist_shallow", roomlist);
		return obj;
	}
